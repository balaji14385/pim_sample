...
function calcProgress(values: FormValues): number {
  let filled = 0;
  if (values.industryId.trim() && !validateField("industryId", values.industryId)) filled++;
  if (values.name.trim() && !validateField("name", values.name)) filled++;
  if (values.code.trim() && !validateField("code", values.code)) filled++;
  if (values.image && !validateField("image", values.image)) filled++;
...
export default function AddCategoryPage() {
  ...
  const progress = calcProgress(values);
    useEffect(()=>{
     async function indList(){
       let data= await fetch('/api/industryList')
  ...
  const fieldState = (field: FieldName): "error" | "ok" | "" => {
    ...
    if (field === "image" ? values.image : (values[field] as string).trim()) return "ok";
    return "";
  };
 ...
 const handleBlur =
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTouched({ industryId: true, name: true, code: true, image: true });
        ...
        />
              ...
              >
                ...
                {!imagePreview ? (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-2 w-full border-2 border-dashed rounded-lg px-4 py-7 transition-all duration-200 ${
                  ...
                  <div className="flex items-center gap-4 w-full bg-slate-50 border border-slate-200 rounded-lg p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        >
                          <Icon d={ICONS.trash} size={11} sw={2} />
                          Remove
                        </button>
                      </div>
                    </div>
            ...
            <div className="flex items-center gap-2.5 mt-7 pt-6 border-t border-slate-100">
              <button
                ...
                md:font-medium md:text-[14px] md:px-6 md:py-2.5 px-3 py-2 font-small text-[9px]
