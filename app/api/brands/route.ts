...
export async function POST(req:NextRequest) {
    try {
        ...
        console.log(data)
        const restorePro = await db.update(brands)
                   .set({ status: true })
                   .where(and(eq(brands.name, data.name),eq(brands.brandCode, data.brandCode))).returning()
               ...
               if (restorePro.length === 0) {
                   ...
                   result = await db.insert(brands).values(
                       finaldata
                   )
               }
